...
const RideHistory1 = () => {
    ...
    const fetchRideHistory = async () => {
      try {
        ...
        const formattedData = data.rideHistory.map((ride) => ({
          ...ride,
          date: new Date(ride?.createdAt._seconds * 1000).toLocaleDateString("en-US", {
            month: "short",
            day: "numeric",
            year: "numeric",
          }),
  ...
  <div key={ride?.id} className="relative bg-white rounded-xl shadow-lg p-6 border border-gray-200 overflow-hidden">
    ...
    <div className="flex justify-between items-center border-b pb-4 mb-4">
      ...
      <div className="flex items-center space-x-2">
        <Calendar className="text-gray-500" size={18} />
        <p className="font-semibold text-gray-700">{moment(ride?.date).format("DD-MM-YYYY")}</p>
      </div>
        <div className="text-gray-500 text-sm">
          <p><strong>Slot:</strong> {ride?.slot?.startTime} - {ride?.slot?.endTime}</p>
          {ride?.slot?.isToday && <p className="text-blue-500 font-medium">Today</p>}
        </div>
      </div>
    ...
    <div className="flex justify-between items-center">
      <p className="text-gray-500 text-sm">Ride ID: <span className="font-semibold text-gray-700">{ride?.rideId}</span></p>
      <div className="flex items-center space-x-2">
        {ride?.status === "Completed" && <CheckCircle className="text-green-500" size={18} />}
        {ride?.status === "Canceled" && <XCircle className="text-red-500" size={18} />}
        {ride?.status === "Ongoing" && <Car className="text-blue-500" size={18} />}
      ...
      <div className="">
        <div className="mt-4 flex items-center space-x-4">
          <div className="flex-1 text-center">
            ...
            <p className="text-xs text-gray-600 mt-1">
              <Clock size={14} className="mr-1 inline" /> {ride?.pickup}
            </p>
          </div>
          ...
          <div className="flex-1 text-center">
            ...
            <p className="text-xs text-gray-600 mt-1">
              <Clock size={14} className="mr-1 inline" /> {ride?.dropoff}
            </p>
          </div>
        </div>
  ...
  <div className="relative bg-white rounded-lg shadow-lg border border-gray-300 px-6 py-4 w-80 text-center">
    ...
    <p className="text-gray-600 text-lg font-semibold">No Rides Found</p>
    <p className="text-gray-500 text-sm mt-1">You haven’t taken any rides yet.</p>
    <div className="border-dashed border-t-2 border-gray-300 mt-4 pt-2">
      <p className="text-gray-400 text-xs">Book a ride to see history here</p>
    </div>
  </div>
</div>
